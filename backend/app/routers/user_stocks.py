...
async def analyze_stock(symbol: str):
    ...
    try:
        ...
        if df is not None and not df.empty:
            idx = df[df["Symbol"].str.upper().str.replace(".NS", "") == canonical_symbol.upper().replace(".NS", "")].index
            if not idx.empty:
                ...
                try:
                    ...
                    if latest_snap:
                        ...
                        WHERE snapshot_id = ? AND (UPPER(symbol) = UPPER(?) OR UPPER(REPLACE(symbol, '.NS', '')) = UPPER(REPLACE(?, '.NS', '')))
                        """
                        conn.execute(
                            update_stock_query,
                            (
                                ...
                                snapshot_id, canonical_symbol, canonical_symbol
                            )
                        )
